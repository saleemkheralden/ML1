...
class KnnClassifier:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        data_matrix = X  # n x d
        train_matrix = self.X  # d x m

        n = data_matrix.shape[0]  # m_test
        m = train_matrix.shape[0]  # m_train

        A = np.asmatrix(list(map(sum, data_matrix * data_matrix))).T.repeat(m, axis=1)
        B = np.asmatrix(list(map(sum, train_matrix * train_matrix))).repeat(n, axis=0)
        AB2 = -2 * (data_matrix @ train_matrix.T)

        mulMatrix = A + AB2 + B

        labelMatrix = np.asmatrix(self.Y).repeat(n, axis=0)

        mat = np.zeros((mulMatrix.shape[0], mulMatrix.shape[1], 2))
        mat[:, :, 0] = mulMatrix
        mat[:, :, 1] = labelMatrix
        mat = np.array(list(map(lambda row: sorted(row, key=lambda e: e[0]), mat)))[:, :self.k, 1]

        return np.array(list(map(lambda row: max(row, key=list(row).count), mat)))
